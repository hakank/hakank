/*
  ...
  Stable marriage problem in ECLiPSe..
...
/*
 ...
 Note: Propia is not used directly in this model, but if
 it's not loaded there is a instantiation fault in arg.
...
all_solutions(Problem) :-
        printf("\nProblem %d:\n", [Problem]),
        findall([Husband,Wife,Backtracks], stable_marriage(Problem,Husband,Wife,Backtracks),L),
        ( foreach([H,W,B], L) do
              writeln(husband:H),
              writeln(wife:W),
...
stable_marriage(Problem,Husband,Wife, Backtracks) :-

        problem(Problem, RankWomen,RankMen),

        dim(RankWomen,[NumWomen,NumMen]),

        dim(RankMen,[NumMen,NumWomen]),
        dim(Wife,[NumMen]),
        Wife #:: 1..NumWomen,
        ...
        ( for(M,1,NumMen) * for(O,1,NumWomen),
          param(RankMen,RankWomen,Wife,Husband) do
