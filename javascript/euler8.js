/* 

  Euler #8 in JavaScript.

  Problem 8:
  """
  Find the greatest product of five consecutive digits in the 
  1000-digit number.
  ...
  """

  This JavaScript program was created by Hakan Kjellerstrand, hakank@gmail.com
  See also my JavaScript page: http://www.hakank.org/javascript/

*/

'use strict';
var {range,prod,prod2,timing2} = require('./js_utils.js');

// Keep it global so we don't get sore eyes...
var n = "";

// 0ms
var euler8a = function() {
    var a = n.split("").map(i=>parseInt(i))
    var len = a.length;
    var max = 0;
    for(var i = 0; i < len-5+1; i++) {
        var p = 1;
        for (var j = 0; j < 5; j++) {
            p *= a[i+j];
            if (p===0) {
                continue;
            }
        }
        if (p > max) {
            max = p;
        }
    }
    return max;
}

// 1ms
var euler8b = function() {
    var a = n.split("").map(i=>parseInt(i))
    var len = a.length;
    var max = 0;
    for(let i = 0; i < len-5+1; i++) {
        var p = prod(a.slice(i,i+5));        
        if (p > max) {
            max=p;
        }
    }
    return max;
}

// 0ms
var euler8c = function() {
    var a = n.split("").map(i=>parseInt(i))
    return range(a.length-5+1)
        .map(i=> {
            return prod(a.slice(i,i+5));        
        })
        .max2();
}

// 0ms
// Not very functional programming...
var euler8d = function() {
    var a = n.split("").map(i=>parseInt(i));
    var max = 0;
    while (a.length > 0) {
        var p = prod(a.slice(0,5));
        a.shift()        
        if (p > max) {
            max = p;
        }
    }
    return max;
}


timing2(euler8a);
// timing2(euler8b);
// timing2(euler8c);
// timing2(euler8d);
