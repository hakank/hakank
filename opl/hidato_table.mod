/* 
  ...
  This version use the table constraint (allowedAssignments).
...
// The connections, used with allowedAssignments
{A} valid_connections =
  {<i1, j1, i2, j2> | i1 in 1..r, j1 in 1..c, i2 in 1..r, j2 in 1..c:
       abs(i1-i2) <= 1 && abs(j1-j2) <= 1 && !(i1 == i2 && j1 == j2)};
...
constraints {
  ...
  forall(k in 1..r*c-1) {
        ...
        allowedAssignments(valid_connections, itmp[k],  jtmp[k],  itmp[k]+atmp[k],  jtmp[k]+btmp[k]);
