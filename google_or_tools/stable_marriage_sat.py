...
"""
  ...
  Stable marriage problem in OR-tools CP-SAT Solver.
  ...
  This is a port of my old CP model stable_marriage.py,
  though I added some more instances: problems 5, 6, 7,
  and a random generator (with no guarantee of a solution).

  Note: The problem instances are represented as 1..n, but we need
        to reformutate this to 0..n-1 in order to use Element, 
        and in the output we show 1..n again.
...
class SolutionPrinter(cp.CpSolverSolutionCallback):
    ...
    def OnSolutionCallback(self):
        ...
        # Convert back to 1..n format
        print("wife   : ", [self.Value(w)+1 for w in self.__wife])
        print("husband: ", [self.Value(h)+1 for h in self.__husband])
        print()
...
def random_instance(n=10):
  """
  random_instance(n)

  Create a random problem instance of size `n`.

  Note: There is no guarantee that it has a solution.
  """
  rankMen = []
  rankWomen = []
  for _i in range(n):
    sMen = list(range(1,n+1))
    shuffle(sMen)
    rankMen.append(sMen)

    sWomen = list(range(1,n+1))
    shuffle(sWomen)
    rankWomen.append(sWomen)
...
def stable_marriage(ranks, problem_name):
  """
  Main run of a problem
  """
  ...
  # Convert to 0..n-1 format
  rankMen =   [[v-1 for v in t] for t in ranks["rankMen"]]
  rankWomen = [[v-1 for v in t] for t in ranks["rankWomen"]]
  ...
  # The "forall ... cp.post" comments below are from my Comet model.

  ## These following three constraints 2 alldifferent and inverse
  ## are redundant and are in fact slower than using the two 
  ## constraints:
  ##    model.AddElement(wife[m],husband, m)
  ##    model.AddElement(husband[w],wife,w)

  #
  # model.AddAllDifferent(wife)
  # model.AddAllDifferent(husband)
  #
  ## if w if the wife of h then h is the husband of w
  # model.AddInverse(wife,husband)

  #
  # This also implements the inverse constraint
  # and is faster.
  #
  # forall(m in Men)
  ...
  for m in range(n):
    for o in range(n):
      ...
      t1 = model.NewIntVar(0,n-1,"t1")
      model.AddElement(wife[m],rankMen[m], t1)
      model.Add(rankMen[m][o] < t1).OnlyEnforceIf(b1)
      model.Add(rankMen[m][o] >= t1).OnlyEnforceIf(b1.Not())
      
      b2 = model.NewBoolVar("b2")
      t2 = model.NewIntVar(0,n-1,"t2")
      model.AddElement(husband[o], rankWomen[o], t2)
      model.Add(t2 < rankWomen[o][m]).OnlyEnforceIf(b2)
      model.Add(t2 >= rankWomen[o][m]).OnlyEnforceIf(b2.Not())
  ...
  for w in range(n):
    for o in range(n):
      b1 = model.NewBoolVar("b1")
      t1 = model.NewIntVar(0,n-1,"t1")
      model.AddElement(husband[w], rankWomen[w], t1)
      model.Add(rankWomen[w][o] < t1).OnlyEnforceIf(b1)
      model.Add(rankWomen[w][o] >= t1).OnlyEnforceIf(b1.Not())

      b2 = model.NewBoolVar("b2")
      t2 = model.NewIntVar(0,n-1,"t2")
      model.AddElement(wife[o],rankMen[o], t2)
      model.Add(t2 < rankMen[o][w]).OnlyEnforceIf(b2)
      model.Add(t2 >= rankMen[o][w]).OnlyEnforceIf(b2.Not())
...
problem5 = {
  "rankMen":
   [[10,2,5,9,8,1,4,7,3,6], [8,9,1,3,7,4,10,2,5,6], [8,3,10,2,9,7,4,1,5,6],
    [5,2,10,7,4,6,3,1,9,8], [3,9,4,1,7,5,2,6,8,10], [9,8,7,4,10,2,6,1,3,5],
    [8,3,9,4,6,7,10,1,5,2], [8,1,7,4,2,3,10,9,5,6], [8,3,6,4,2,7,9,1,10,5],
    [6,8,5,2,3,1,10,4,7,9]],
  "rankWomen":
   [[10,4,9,7,6,1,3,8,2,5], [5,9,4,7,8,10,6,1,3,2], [2,7,5,4,1,10,8,3,6,9],
    [3,2,10,7,4,8,6,5,9,1], [9,1,8,3,5,2,4,7,6,10], [6,7,5,8,1,3,4,9,10,2],
    [9,2,8,3,4,10,1,7,6,5], [8,4,10,2,9,6,5,3,1,7], [6,7,1,8,4,2,9,10,3,5],
    [7,9,5,8,6,10,3,2,4,1]]
}

# Random 20x20 instance (from my MiniZinc instance
...
# 
# Random 50x50 instance from my MiniZinc instance
...
if __name__ == "__main__":
  stable_marriage(van_hentenryck, "Van Hentenryck")
  stable_marriage(mathworld, "MathWorld")
  stable_marriage(problem3, "Problem 3")
  stable_marriage(problem4, "Problem4")
  stable_marriage(problem5, "Problem5")
  stable_marriage(problem6, "Problem6")
  stable_marriage(problem7, "Problem7")
  # stable_marriage(random_instance(100),"Random") # run a random instance
