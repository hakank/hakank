...
"""
  ...
  This is a port of my old CP model furniture_moving.py
...
def main():
  ...
  #
  for i in range(n):
    model.Add(end_times[i] == start_times[i] + duration[i])

  model.AddMaxEquality(end_time,end_times)

  my_cumulative(model, start_times, duration, demand, num_resources)
