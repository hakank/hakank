/*
  ...
  Set covering, scheduling in B-Prolog.
...
go :-
        ...
        % decision variables
        length(X, NumCombinations),
        X :: 0..1,

        length(PreferencePoints, NumPersons),
        ...
        foreach(P in 1..NumPersons,
                (
                    ...
                    PreferencePoints[P] #=  sum([(X[I]*Preferences[P,J]) : 
        ...
        foreach(J in 1..NumTasks,
                sum([(X[I]) : I in 1..NumCombinations,
                     [CI], (CI @= Comb[I], J in CI)]) #= 1
               ),

        term_variables([X,PreferencePoints], Vars),
        % satisfy the employee's preferences as much as possible
        maxof(labeling(Vars),SumPrefPoints),
        ...
        foreach(I in 1..NumCombinations,
                ...
                (
                    X[I] #= 1 -> 
                        ...
                        P @= Persons[CPI],
                        C @= Comb[I],
                        format("Combination ~d: ~w Tasks: ~w\n", [I,P,C])
                ;
                        true
                )),
