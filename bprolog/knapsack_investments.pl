/*
  ...
  Knapsack (investment) problem in B-Prolog.

  From the Swedish book
  Lundgren, Rönnqvist, Värbrand: 
  "Optimeringslära" (Optimization theory),
  page 393ff.
...
go :-
        ...
        % search
        maxof(labeling([ff,down], X), TotalValues,
              format("TotalValue: ~d\n",[TotalValues])
             ),
        ...
        foreach((XX,P) in (X,1..NumProjects),
                (XX == 1 ->
                     write(P), write(' ')
                ;
                     true
