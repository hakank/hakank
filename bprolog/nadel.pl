/*
  ...
  Nadel's construction problem in B-Prolog.
...
go :-
        findall([Developments,Broken,TotalCount],
                 ...
                 List),
        length(List,Len),
        foreach([Developments,Broken,TotalCount] in List,
                ac(AllBroken,[]),
                (
                    AllBroken^1 = [Broken|AllBroken^0],
        ...
        TLen @= AllBrokenTransposed^length,
        foreach((B,C) in (AllBrokenTransposed,1..TLen),
                [Occurrences],
                ( Occurrences = sum(B),
